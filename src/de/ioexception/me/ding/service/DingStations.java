...
public class DingStations
{
	...
	/**
	 * Returns a limited list of near stations based on the position.
	 * 
	 * @param position
	 * @param limit
	 * @return a vector of {@link ProximitySearchResult}s, starting with the
	 *         best match
	 */
	public static Vector getStationsByPosition(Wgs84Coordinate position, int limit)
	{
		Vector hits = new Vector();
		Vector distanceVector = new Vector();
		
		for(Enumeration e = stations.keys(); e.hasMoreElements();)
		{
			String id = (String) e.nextElement();
			BusStation station = (BusStation) stations.get(id);
			distanceVector.addElement(new ProximitySearchResult(station, position));
		}
		...
		while(!sorted)
		{
			...
			for(int i = 0; i < distanceVector.size() - 1; i++)
			{
				ProximitySearchResult current = (ProximitySearchResult) distanceVector.elementAt(i);
				ProximitySearchResult next = (ProximitySearchResult) distanceVector.elementAt(i + 1);
				
				if(current.compare(next) > 1)
				{
					distanceVector.setElementAt(next, i);
					distanceVector.setElementAt(current, i + 1);
					sorted = false;
				}
			}
		}

		for(int i = 0; i < distanceVector.size() && i < limit; i++)
		{
			hits.addElement(distanceVector.elementAt(i));
		}
