 ...
 * response page, passing it to a {@link DingScheduleCallback}.
 * 
 * @author Benjamin Erb
 */
class DingScheduleResponseListener extends DefaultHandler implements HttpResponseListener
{
	private final DingScheduleCallback callback;
	private final Vector results;

	private final Stack currentElement = new Stack();

	public DingScheduleResponseListener(DingScheduleCallback callback)
	{
	...
	private static class HtmlAjaxResponseHandler extends DefaultHandler
	{
		...
		private void submit()
		{
			...
			{
				results.addElement(new DingScheduleResult(target, line, dept));
			}
